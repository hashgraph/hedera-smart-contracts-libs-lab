...
import {
    ...
    FileAppendTransaction, ContractId,
} from "@hashgraph/sdk";
...
const contractCreateGas = 80_000;
const contractExecGas = 40_000;
...
async function main() {
    ...
    // const contractId = ContractId.fromString("0.0.2166612");
    ...
    const functionCallAsUint8Array = encodeFunctionParameters('tryAlternative',  [hashedMessage, V, R, S]);
    ...
    const contractCall = await new ContractExecuteTransaction()
        ...
        .setGas(contractExecGas)
        .execute(client);

    const record = await contractCall.getRecord(client);

    let results = abiInterface.decodeFunctionResult('tryAlternative', record.contractFunctionResult.asBytes());
